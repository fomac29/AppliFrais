	...
	public function _remap($action, $params = array())
	{
		...
		else
		{
			...
			elseif ($action == 'mesFiches')		// mesFiches demandé : on active la fonction mesFiches du modèle utilisateur
			{
				...
				$this->a_utilisateur->mesFiches($idVisiteur);
			}
			...
			{	// TODO : contrôler la validité du second paramètre (mois de la fiche à consulter)
				...
				$this->a_utilisateur->voirFiche($idVisiteur, $mois);
			}
			...
			{	// TODO : contrôler la validité du second paramètre (mois de la fiche à modifier)
				...
				$this->a_utilisateur->modFiche($idVisiteur, $mois);
			}
			...
			{	// TODO : contrôler la validité du second paramètre (mois de la fiche à modifier)
				...
				$idVisiteur = $this->session->userdata('idUser');
				$this->a_utilisateur->signeFiche($idVisiteur, $mois);

				// ... et on revient à mesFiches
				$this->a_utilisateur->mesFiches($idVisiteur, "La fiche $mois a été signée. <br/>Pensez à envoyer vos justificatifs afin qu'elle soit traitée par le service comptable rapidement.");
			}
			...
			{	// TODO : conrôler que l'obtention des données postées ne rend pas d'erreurs
				...
				$this->a_utilisateur->majForfait($idVisiteur, $mois, $lesFrais);

				// ... et on revient en modification de la fiche
				$this->a_utilisateur->modFiche($idVisiteur, $mois, 'Modification(s) des éléments forfaitisés enregistrée(s) ...');
			}
			...
			{	// TODO : conrôler que l'obtention des données postées ne rend pas d'erreurs
				...
				$this->a_utilisateur->ajouteFrais($idVisiteur, $mois, $uneLigne);

				// ... et on revient en modification de la fiche
				$this->a_utilisateur->modFiche($idVisiteur, $mois, 'Ligne "Hors forfait" ajoutée ...');				
			}
			...
			{	// TODO : contrôler la validité du second paramètre (mois de la fiche à modifier)
				...
				$this->a_utilisateur->supprLigneFrais($idVisiteur, $mois, $idLigneFrais);

				// ... et on revient en modification de la fiche
				$this->a_utilisateur->modFiche($idVisiteur, $mois, 'Ligne "Hors forfait" supprimée ...');				
			}
